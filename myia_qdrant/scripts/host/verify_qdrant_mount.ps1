...
.DESCRIPTION
    ...
    Avec -Remediate : sur drift (codes 1-4) OU container down (code 9), declenche la tache
    scheduled Mount-Qdrant-VHDX (qui a RunLevel=Highest) via schtasks. Pas d'UAC popup :
    ...
    Lecon 2026-05-28 (2 containers REMOVED en 8h apres hard crashes Windows) :
    le watchdog detectait correctement code 9 = container down, mais TryRemediate
    excluait code 9 de l'auto-repair -> WARN sans declencher Mount-Qdrant-VHDX,
    intervention humaine requise. Code 9 ajoute aux codes remediate-eligibles.
.CATEGORY
    monitoring
.REQUIRES
...
.NOTES
    Sortie codes:
      ...
      9   = container down (REMOVED ou stopped) avec mount OK  (REMEDIATE-ELIGIBLE depuis 2026-05-28)
    ...
    Update 2026-05-28 (post 2 containers REMOVED en 8h apres hard crashes Windows) :
      - TryRemediate accepte code 9 (CONTAINER_DOWN). Mount-Qdrant-VHDX est idempotent :
        Phase 1-5 skip si mount OK, Phase 6 = compose up (re-cree le container manquant).
        Le compose-up retry de Mount-Qdrant-VHDX (defense-in-depth) couvre la fenetre
        etroite ou la socket bind-mount est mid-init.
#>
